...
public:
    ...
    void em_fdb_callback(const wtr_serial::em_fb_raw &joint_state);
    void em_cmd_send();
};

FSM_topic_control::FSM_topic_control(ros::NodeHandle &nh,FSM_data &data):nh_(nh)
{
    data_ = &data;
    this->em_cmd_pub_ = this->nh_.advertise<wtr_serial::em_ev>("/em_ev", 1);
...
void FSM_topic_control::em_fdb_callback(const wtr_serial::em_fb_raw &msg){
    data_->mt_fdb = msg;
    data_->UpdateData();
    return;
}
