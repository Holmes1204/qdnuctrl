...
public:
    ...
    /*! Callback Function */
    void MotorCallback(const wtr_serial::em_fb_raw &msg);
...
FSM::FSM(ros::NodeHandle &nh) {
    ...
    //motor control 
    this->angle_real_pub_ = this->nh_.advertise<wtr_serial::em_ev>("/em_ev", 1);
    ...
    //motor feedback topic 
    this->motor_sub_ = this->nh_.subscribe("/em_fb_raw", 2, &FSM::MotorCallback ,this);
...
void FSM::SendCommand(){
    ...
    for (int i = 0; i < 4; i++) {
        ...
        //2 -> DOWN
        //MID
        motor_cmd_.em_ev_pos[i*2]   =this->state_interior_.foot_qDes(1, i); 
        motor_cmd_.em_ev_vel[i*2]   =this->state_interior_.foot_qdDes(1,i);
        motor_cmd_.em_ev_trq[i*2]   =0;
        motor_cmd_.em_ev_kp[i*2]    = 50;
        motor_cmd_.em_ev_kd[i*2]     = 2;
        //DOWN
        motor_cmd_.em_ev_pos[i*2+1] =this->state_interior_.foot_qDes(2, i);   
        motor_cmd_.em_ev_vel[i*2+1] =this->state_interior_.foot_qdDes(2,i);  
        motor_cmd_.em_ev_trq[i*2+1] =0;
        motor_cmd_.em_ev_kp[i*2+1] =50;  
        motor_cmd_.em_ev_kd[i*2+1] =2;     
    }
    angle_real_pub_.publish(motor_cmd_);
}

void FSM::MotorCallback(const wtr_serial::em_fb_raw &msg) {
    for (int i = 0; i < NUM_LEG; i++)
    {
        this->state_interior_.foot_q(1,i)  = msg.em_pos_fb_raw[i*2];
        this->state_interior_.foot_qd(1,i) = msg.em_vel_fb_raw[i*2];
        this->state_interior_.foot_q(2,i)  = msg.em_pos_fb_raw[i*2+1];
        this->state_interior_.foot_qd(2,i) = msg.em_vel_fb_raw[i*2+1];
    }
}
